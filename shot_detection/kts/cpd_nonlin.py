...
def seg_cpd_nonlin(K, ncp, max_overlap, lmin=1, lmax=100000, backtrack=True, verbose=True,
               out_scatters=None):
    """Change point detection with dynamic programming
    ...
    if m>0: 
        ...
        overlap=0
        if last_pos>0:
            overlap=int((n-last_pos)/2)
            if overlap > max_overlap:
                overlap = max_overlap
            end_scatter=J[n-overlap][n-1]
    else:
        cps=np.zeros(1, dtype=int)
        cps[0] = n-1
        overlap = min(max_overlap, int(n/2))
        end_scatter=J[n-overlap][n-1]
    return cps, J, overlap,  end_scatter
