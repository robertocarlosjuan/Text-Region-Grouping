    ...
    for idx, frame in tqdm(enumerate(frames)):
        ...
        for tracked_coor, value in bbox_dict.items():
            ...
            for curr_bbox in ocr_bboxes:
                ...
                if not check_coor_match(tracked_coor, curr_coor, 5) and idx == frame_range[-1] + 1 and lev(curr_text, tracked_text) < 10:
...
def find_subtitle_bbox(frame_to_subtitle, subtitle_bboxes, frame_range, num_frames, bbox, fps, height):
    ...
    return frame_to_subtitle, is_subtitle

def find_name_bbox(frame_to_name, name_bboxes, frame_range, num_frames, bbox, fps, height, feature_frame_idx, is_subtitle=False):
...
def main(args):
    ...
    frame_to_name = {k:[] for k in range(len(frames))}
    subtitle_bboxes = []
    name_bboxes = []
    ...
    for bbox in bbox_hist:
        ...
        frame_to_subtitle, is_subtitle = find_subtitle_bbox(frame_to_subtitle, subtitle_bboxes, frame_range, num_frames, bbox, fps, height)

        # Detect name bbox
        frame_to_name = find_name_bbox(frame_to_name, name_bboxes, frame_range, num_frames, bbox, fps, height, feature_frame_idx, is_subtitle)
    ...
    for idx, frame in enumerate(frames):
        ...
        #             highest_bbox = bbox
        for bbox in frame_to_name[idx]:
            img = plot_bbox(img, 'Name', bbox, (255, 0, 0))
        out.write(img)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--input_path", type=str, default='../montage_detection/sinovac_ver/ref_video')
    parser.add_argument("--output_path", type=str, default='../non_commit_trg/temp')
    parser.add_argument("--frame_save_path", type=str, default='../non_commit_trg/frames')
